...
const { app, BrowserWindow, globalShortcut, ipcMain, dialog, systemPreferences, nativeTheme } = require('electron');
const path = require('path');
const Store = require('electron-store');
const geminiService = require('./services/gemini');
const meetService = require('./services/meets'); 
...
let mainWindow;
let isQuitting = false; // Added this flag for proper quit handling on macOS
...
function createWindow() {
  ...
  console.log('Content protection is enabled.');
...
// Forward MeetService events to the renderer process
meetService.on('status', (status) => {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('meet-status-update', status);
  }
...
meetService.on('transcript', (transcript) => {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('meet-transcript-update', transcript);
  }
});
meetService.on('error', (error) => {
  const errorMessage = error.message || String(error);
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('meet-error', { message: errorMessage });
  }
});
...
app.whenReady().then(() => {
  // *** CHANGE 1: THE PERMISSION CHECK IS REMOVED FROM HERE ***
  createWindow();
...
ipcMain.handle('set-assemblyai-api-key', (event, apiKey) => {
  try {
    store.set('assemblyAiApiKey', apiKey);
    return meetService.init(apiKey);
  } catch (error) {
    return false;
  }
});
...
ipcMain.on('gemini-prompt', async (event, { view, prompt, messageId }) => {
  try {
    const response = await geminiService.sendMessage(prompt);
    event.sender.send('gemini-response', { view, response, messageId, error: null });
  } catch (error) {
    event.sender.send('gemini-response', { view, response: error.message, messageId, error: true });
  }
});
ipcMain.on('toggle-always-on-top', () => {
  const isAlwaysOnTop = !store.get('isAlwaysOnTop'); // <--- THIS IS THE CORRECT LOGIC
  mainWindow.setAlwaysOnTop(isAlwaysOnTop, 'floating');
  store.set('isAlwaysOnTop', isAlwaysOnTop);
  mainWindow.webContents.send('update-always-on-top-status', isAlwaysOnTop);
});
...
// *** CHANGE 2: THE PERMISSION LOGIC IS MOVED INSIDE 'start-meet' ***
ipcMain.on('start-meet', async () => {
  try {
    ...
    if (process.platform === 'darwin') {
      ...
      if (status === 'not-determined') {
        ...
        if (granted) {
          ...
          dialog.showErrorBox('Permission Denied', 'Microphone access was not granted.');
        }
      } else if (status === 'denied') {
        dialog.showErrorBox('Access Denied', 'Microphone access is denied. You must enable it in System Settings.');
      } else {
        hasPermission = true;
      }
    } else {
      hasPermission = true; // Assume permission for non-macOS
    }
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('meet-error', { message: `Failed to start meet: ${error.message}` });
    }
  }
});
...
ipcMain.on('stop-meet', async () => {
  try {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('meet-error', { message: `Error stopping meet: ${error.message}` });
    }
  }
});